...
function scanBlocksDirectory(blocksPath) {
  const blockTypes = {};
  try {
    fs.readdirSync(blocksPath).forEach((file) => {
      if (file.endsWith(".liquid")) {
        const blockType = path.basename(file, ".liquid");
        blockTypes[blockType] = path.join(blocksPath, file);
      }
    });
  } catch (error) {
...
module.exports = {
  ...
  usePatternLabConfig: function (config) {
    ...
    const supportedPageTypes = ["page", "event", "person"];
    ...
    engine.registerTag("stageblocks", {
      ...
      async render(ctx) {
        ...
        for (const block of object.blocks) {
          ...
          if (templatePath) {
            try {
              output += await this.liquid.renderFile(templatePath, { block });
            } catch (error) {
